...
const FALLBACK_REGULATIONS = ['NAAC', 'NHERC', 'UGC', 'AICTE', 'NBA', 'Other'];
...
const Accreditation = () => {
  ...
  useEffect(() => {
    ...
    const loadAccreditation = async (showLoader = false) => {
      if (showLoader && isMounted) {
        setLoading(true);
      }

      try {
        const response = await dashboardAPI.accreditation();
        if (isMounted) {
          setData(response);
        }
      } catch (error) {
  ...
  const regulations = data.regulations?.length > 0
    ? data.regulations
    : FALLBACK_REGULATIONS.map((regulationType) => ({
