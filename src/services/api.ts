...
const apiFetch = async (url: string, options: RequestInit = {}) => {
  ...
  if (
    (contentDisposition && contentDisposition.toLowerCase().includes("attachment")) ||
    (contentType && !contentType.includes("application/json") && !contentType.startsWith("text/"))
  ) {
...
export const submissionsAPI = {
  ...
  download: (id: number) => apiFetch(`/submissions/${id}/download`),
...
export const dashboardAPI = {
  stats: () => apiFetch(`/dashboard/stats?t=${Date.now()}`, { cache: 'no-store' }),
